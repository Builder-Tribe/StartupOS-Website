    ...
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-200/80 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="md:hidden flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab('launchpad')}>
            <div className="w-8 h-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white font-black text-sm">
              S
            </div>
            <span className="font-extrabold text-base tracking-tight text-slate-900">StartupOS</span>
          </div>

          {/* Active Workspace Selector Indicator */}
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100/90 border border-slate-200 text-xs font-semibold text-slate-700">
            ...
            <span>{currentUser.workspaceName || `${currentUser.name}'s Studio`}</span>
        ...
        {/* Right: Quick Launch & User Profile */}
        <div className="flex items-center gap-3">
          <button
            onClick={onOpenLaunchModal}
            className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3.5 py-2 rounded-xl shadow-xs transition-all active:scale-95 md:hidden"
          >
