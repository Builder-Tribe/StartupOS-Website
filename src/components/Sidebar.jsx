...
export default function Sidebar({ 
  ...
  return (
    <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col justify-between shrink-0 border-r border-slate-800 hidden md:flex sticky top-0 h-screen select-none z-30">
      ...
      {/* Bottom Footer Section: Profile, Logout & Help */}
      <div className="p-3.5 border-t border-slate-800/80 space-y-2.5 bg-slate-900/90">
        {/* Active User Card with Explicit Logout */}
        {isLoggedIn && currentUser && (
          <div className="p-2.5 rounded-xl bg-slate-800/80 border border-slate-700/60 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 overflow-hidden min-w-0">
              <span className="w-7 h-7 rounded-lg bg-indigo-950 border border-indigo-700 flex items-center justify-center text-xs shrink-0">
                {currentUser.avatar || '👩‍💻'}
              </span>
              <div className="truncate">
                <span className="text-xs font-bold text-white block truncate">{currentUser.name}</span>
                <span className="text-[10px] text-slate-400 font-medium block truncate">
                  {currentUser.role === 'admin' ? 'Super Admin' : (currentUser.badge || 'Builder')}
                </span>
              </div>
            </div>
            ...
            {onLogout && (
              <button
                onClick={onLogout}
                className="p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-500/15 transition-all cursor-pointer shrink-0"
        ...
        {!isLoggedIn && onOpenAuthModal && (
          <button
            ...
            className="w-full p-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md shadow-indigo-600/20"
          >
        ...
        <div 
          ...
          className="p-2.5 rounded-xl bg-gradient-to-br from-indigo-950/80 to-slate-800 border border-indigo-500/30 hover:border-indigo-400 transition-all cursor-pointer group shadow-xs"
        >
          <div className="flex items-center justify-between text-xs mb-0.5">
            <span className="text-white font-bold flex items-center gap-1.5 text-[11px]">
              <HelpCircle className="w-3.5 h-3.5 text-indigo-400" /> Need Help?
            </span>
          ...
          <p className="text-[10px] text-slate-300 font-medium leading-relaxed">
            Get 1-on-1 AI assistance & governance answers.
          </p>
        </div>
