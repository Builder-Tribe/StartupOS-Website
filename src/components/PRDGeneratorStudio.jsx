    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 mt-2 tracking-tight flex items-center gap-3">
            <FileCode className="w-8 h-8 text-indigo-600" />
            AI Specification Package
          </h1>
          <p className="text-slate-600 text-sm mt-1">
            Generating production-ready PRD, Market Analysis, and Technical Architecture docs for: <strong className="text-indigo-700 font-bold">{currentWorkspace.name}</strong>
          </p>
        </div>

        {/* Workspace Selector dropdown */}
        <div className="flex items-center gap-3 bg-white border border-slate-200/90 p-2 px-3 rounded-xl shadow-xs">
          ...
          <select
            ...
            className="bg-transparent text-slate-800 text-xs font-bold focus:outline-none pr-2 cursor-pointer"
          >
            {ideas.map((item) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <aside className="lg:col-span-4 bg-white border border-slate-200/90 rounded-2xl p-4 space-y-2 h-fit shadow-sm">
          <h3 className="text-xs font-mono uppercase text-slate-500 font-bold px-3 py-1">Generated Documentation Files</h3>
          {[
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3 rounded-xl border transition-all ${
                  isSelected
                    ? 'bg-indigo-50/90 border-indigo-500 shadow-sm text-slate-900 ring-1 ring-indigo-400'
                    : 'bg-white border-slate-200/80 text-slate-600 hover:text-slate-900 hover:bg-slate-50/60'
                }`}
              >
                <div className="font-mono text-xs font-bold text-indigo-700">{doc.label}</div>
                <div className="text-[11px] text-slate-500 mt-0.5">{doc.desc}</div>
              </button>
            );
          })}
        </aside>
        ...
        <main className="lg:col-span-8 bg-white border border-slate-200/90 rounded-2xl p-6 sm:p-8 shadow-sm">
          <div className="flex justify-between items-center border-b border-slate-200 pb-4 mb-6">
            <span className="text-xs font-mono text-indigo-700 font-bold flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-emerald-600" /> AI-Agent Ready Markdown
            </span>
            <button
              onClick={() => handleCopy(generatedDocs[activeDoc])}
              className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg flex items-center gap-1.5 shadow-xs transition-all"
            >
          ...
          <pre className="bg-slate-900 p-6 rounded-xl text-xs font-mono text-slate-200 border border-slate-800 whitespace-pre-wrap leading-relaxed overflow-x-auto">
